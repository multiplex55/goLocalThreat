...
import { LocalScreen, formatDetailStatusChip, formatRosterSummaryChip } from '../LocalScreen';
...
function buildState(): AnalyzeState {
  return {
    ...
    data: {
      ...
      diagnostics: { candidateNamesCount: 1, resolvedCount: 1, unresolvedNames: [], invalidLines: 0, warnings: [], globalWarnings: [], warningsByPilotId: {}, severityCounts: { info: 0, warn: 0, error: 0 }, providerCounts: {}, warningCodeCounts: {} },
...
describe('LocalScreen layout regions', () => {
  it('keeps center grid and diagnostics strip mounted', () => {
    ...
    expect(screen.getByTestId('diagnostics-expander')).not.toHaveAttribute('open');
    expect(screen.getByTestId('status-expander')).not.toHaveAttribute('open');
  });

  it('updates layout mode attr to wider center when intake drawer collapses', () => {
    render(<LocalScreen pastedText="" analyzeState={buildState()} onPasteChange={() => {}} onAnalyze={() => {}} useLocalIntelV2Layout />);

    fireEvent.click(screen.getByTestId('roster-toggle'));
    expect(screen.getByTestId('local-layout-grid')).toHaveAttribute('data-roster-collapsed', 'false');

    fireEvent.click(screen.getByTestId('roster-toggle'));
    expect(screen.getByTestId('local-layout-grid')).toHaveAttribute('data-roster-collapsed', 'true');
  });

  it('generates summary chip labels', () => {
    expect(formatRosterSummaryChip(5, 3)).toBe('Roster · 5 parsed · 3 resolved');
    expect(formatDetailStatusChip(2, 5)).toBe('detail 2/5');
  });
});
